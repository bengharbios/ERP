        ...
        if (req.user) {
            await prisma.auditLog.create({
                data: {
                    ...
                    afterData: JSON.parse(JSON.stringify(finalClassData)),
                    ipAddress: req.ip,
                    userAgent: req.get('user-agent'),
                },
            });
        ...
        if (req.user) {
            await prisma.auditLog.create({
                data: {
                    ...
                    afterData: { instructorId: validatedData.instructorId },
                    ipAddress: req.ip,
                    userAgent: req.get('user-agent'),
                },
            });
        ...
        if (req.user) {
            await prisma.auditLog.create({
                data: {
                    ...
                    afterData: { studentId: validatedData.studentId, classId },
                    ipAddress: req.ip,
                    userAgent: req.get('user-agent'),
                },
            });
        ...
        if (req.user) {
            await prisma.auditLog.create({
                data: {
                    ...
                    resourceType: 'Class',
                    resourceId: id,
                    afterData: { ...validatedData, result },
                    ipAddress: req.ip,
                    userAgent: req.get('user-agent'),
                },
            });
