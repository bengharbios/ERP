...
async function getDynamicBot() {
    ...
    botInstance.on('text', async (ctx: any) => {
        ...
        } else if (text === 'ℹ️ دليل الاستعلامات' || text === '/help' || text === 'مساعدة') {
            const helpMsg = `ℹ️ <b>دليل أوامر واستعلامات نظام السلام CRM الذكي 2026:</b> ℹ️\n\n` +
            `🔑 <b>1. الأوامر الأساسية والتسجيل:</b>\n` +
            `• <code>/login [المستخدم] [المرور]</code> : لتسجيل الدخول وربط حساب التليجرام الخاص بك بالنظام.\n` +
            `• <code>/logout</code> : لتسجيل الخروج وإلغاء ربط الحساب بجهازك.\n` +
            `• <code>/menu</code> أو <code>قائمة</code> : لعرض لوحة المفاتيح التفاعلية الرئيسية في أي وقت.\n\n` +
            `🔍 <b>2. البحث وإدارة العملاء:</b>\n` +
            `• <b>🔍 بحث عن عميل :</b> للبحث برقم الهاتف الفوري وعرض سجل العميل المالي والاتصالي بالكامل مع كاشف التكرار.\n` +
            `• <b>📝 إضافة تقرير :</b> لإضافة عميل جديد أو تحديث ملاحظات ومراحل عميل حالي، مع الترحيل اللحظي لجوجل شيت.\n\n` +
            `📞 <b>3. طابور الاتصالات اليومية:</b>\n` +
            `• <b>📞 اتصالات اليوم (أو /today) :</b> يفتح لك طابور الاتصالات التفاعلي للعملاء المجدول تواصلهم اليوم، مع أزرار للواتساب وتحديث الحالة.\n\n` +
            `📊 <b>4. التقارير والإحصائيات التحليلية:</b>\n` +
            `• <b>📊 ملخص اليوم :</b> يعرض ملخصاً سريعاً لعدد العملاء والتقارير الجديدة المسجلة خلال اليوم.\n` +
            `• <b>🏆 لوحة الصدارة (أو /stats) :</b> يعرض قائمة المتصدرين في المبيعات وإضافة العملاء خلال آخر 7 أيام (مخصص للمدراء).\n` +
            `• <code>/stats_detailed [التاريخ]</code> : يعرض إحصائيات متقدمة للعملاء الفريدين، المهتمين، وتوزيع المراحل (لم يرد، استفسار...) لأي يوم محدد أو فترة (أمثلة: <code>/stats_detailed اليوم</code> أو <code>/stats_detailed أمس</code>).\n\n` +
            `⚙️ <b>5. فحص النظام:</b>\n` +
            `• <b>⚙️ حالة الربط :</b> يفحص اتصال قواعد البيانات، بوت التليجرام، وجوجل شيت.\n\n` +
            `💡 <i>نصيحة: يمكنك إرسال رقم هاتف أي عميل مباشرة في أي وقت للبحث عنه دون الحاجة لضغط أي زر!</i>`;
            ...
            await ctx.reply(
                'مرحباً بك في نظام السلام CRM المتكامل الذكي! 📱💎\n\nيرجى اختيار الإجراء المطلوب من الأزرار بالأسفل:',
                {
                    reply_markup: {
                        keyboard: [
                            [{ text: '🔍 بحث عن عميل' }, { text: '📝 إضافة تقرير' }],
                            [{ text: '📞 اتصالات اليوم' }, { text: 'ℹ️ دليل الاستعلامات' }],
                            [{ text: '📊 ملخص اليوم' }, { text: '🏆 لوحة الصدارة' }],
                            [{ text: '⚙️ حالة الربط' }]
                        ],
                        resize_keyboard: true
                    }
