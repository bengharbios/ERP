...
export const crmService = {
    ...
    parseTelegramMessage(text: string) {
        ...
        const patterns = {
            name: /(?:👤\s*)?الاسم\s*(?::|\s+)\s*(.+)/,
            phone: /(?:📞\s*)?رقم الهاتف\s*(?::|\s+)\s*([\d\s+-]+)/,
            nationality: /(?:🌍\s*)?الجنسي[ةه]\s*(?:\.\.|\s*:\s*|\s+)\s*(.+)/,
            emirate: /(?:📍\s*)?الإمار[ةه]\s*(?::|\s+)\s*(.+)/,
            diploma: /(?:🎓\s*)?الدبلوم(?:\s+المهتم)?(?:\s+به)?\s*(?::|\s+)\s*(.+)/,
            interestLevel: /(?:🔥\s*)?درجة الاهتمام\s*(?::|\s+)\s*(\d+)/,
            notes: /(?:📝\s*)?(?:الملاحظات|الملاحظة|ملاحظات)\s*(?::|\s+)\s*([\s\S]+?)(?=-{3,}|👤|📅|$)/,
            reschedule: /(?:📅\s*)?\*?\s*إعادة جدولة\s*(?::|\s+)\s*(.+)/,
        };
