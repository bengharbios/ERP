...
async function checkDeviceTime() {
    ...
    async function req(url: string, method: 'GET' | 'POST', body?: any) {
        try {
    ...
    try {
        ...
        console.log('--- Last 5 Raw Events on Device ---');
        const list = events?.AcsEvent?.InfoList || [];
        list.forEach((e: any) => console.log(`Emp: ${e.employeeNoString} | Time: ${e.time || e.eventTime} | Name: ${e.name || 'N/A'}`));
...
checkDeviceTime().catch(console.error).finally(() => process.exit());
