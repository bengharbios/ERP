...
async function checkBlockingRecords() {
  ...
  try {
    ...
    console.log(`Attendance Records for this enrollment: ${attendanceRecords.length}`);

    const otherAttendanceRecords = await prisma.attendanceRecord.findMany({
      where: { studentId: student.id }
    });
    console.log(`Total Attendance Records for this student: ${otherAttendanceRecords.length}`);

    const studentAssignments = await prisma.studentAssignment.findMany({
      where: { studentEnrollmentId: enrollment.id }
    });
    console.log(`Student Assignments for this enrollment: ${studentAssignments.length}`);

    const otherStudentAssignments = await prisma.studentAssignment.findMany({
      where: { studentId: student.id }
    });
    console.log(`Total Student Assignments for this student: ${otherStudentAssignments.length}`);
